...
const ChromaGrid: React.FC<ChromaGridProps> = ({
  ...
  const demo: ChromaItem[] = [
    ...
    // Tambahkan 3 item lagi untuk total 9
    {
  ...
  const handleCardClick = (url?: string) => {
    if (url) window.open(url.trim(), '_blank', 'noopener,noreferrer');
  };
  ...
  return (
    <div
      ...
      className={`relative w-235 h-200 grid grid-cols-3 gap-4 justify-items-center animate-fade-in-up ${className}`}
      style={
        {
          '--r': `${radius}px`,
          '--x': '50%',
    ...
    >
      {data.slice(0, 9).map((c, i) => (
        <article
          ...
          className="group relative flex flex-col rounded-[20px] overflow-hidden border-2 border-transparent transition-colors duration-300 cursor-pointer min-h-[320px]"
          style={
            {
